...
:readmestartpage
  echo !define README_STARTPAGE "%~1" >> "%IWMAKE_NSISCONF%"
goto :eof
...
:welcometitle
  echo !define WELCOME_TITLE "%~1" >> "%IWMAKE_NSISCONF%"
goto :eof

:welcomeicon
  echo !define WELCOME_ICON "%~1" >> "%IWMAKE_NSISCONF%"
goto :eof

:finishtitle
  echo !define FINISH_TITLE "%~1" >> "%IWMAKE_NSISCONF%"
goto :eof
...
:buildDir
  ...
  for /F "eol=- tokens=1,2,3" %%m in ('%WINDIR%\system32\find "QT_PACKAGEDATE_STR" %IWMAKE_ROOT%\%IWMAKE_TMP2%\src\corelib\global\qglobal.h') do echo !define MODULE_LICENSECHECK_PACKAGEDATE %%o >> "%IWMAKE_NSISCONF%"
goto :eof
