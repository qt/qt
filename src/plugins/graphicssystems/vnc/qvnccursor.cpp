...
QVNCCursor::QVNCCursor(QVNCServer * srvr, QVNCGraphicsSystemScreen *scr )
        :useVncCursor(false), currentRect(QRect()),
        prevRect(QRect()), server(srvr), screen(scr)
{
    d_ptr = new QVNCCursorPrivate(0, 0, 0, 0, 0, 0);
}
...
{
    d_ptr->set(data, mask, width, height, hotX, hotY);
    if (useVncCursor) {
        sendClientCursor();
    }
}

QRect QVNCCursor::dirtyRect()
{
    if (!useVncCursor) {
        QRect rect = prevRect;
        prevRect = QRect();
...
QRect QVNCCursor::drawCursor(QPainter & painter)
{
    ...
    painter.drawImage(prevRect, *d_ptr->image());
    return prevRect;
}

void QVNCCursor::pointerEvent(QMouseEvent & e)
{
    QWidget *widget = QApplication::widgetAt(e.pos());
    ...
    if (!useVncCursor) {
        currentRect = d_ptr->image()->rect().translated(-d_ptr->hotspot().x(), -d_ptr->hotspot().y());
        currentRect.translate(e.pos());
        screen->setDirty(QRect());
    }
...
void QVNCCursor::sendClientCursor()
{
    ...
    // FramebufferUpdate header
    {
        const quint16 tmp[6] = { htons(0),
                                 htons(1),
                                 htons(d_ptr->hotspot().x()), htons(d_ptr->hotspot().y()),
                                 htons(image->width()),
                                 htons(image->height()) };
        socket->write((char*)tmp, sizeof(tmp));
...
// from src/gui/embedded/QVNCCursorPrivate_qws.cpp

static QVNCCursorPrivate *systemCursorTable[Qt::LastCursor+1];
...
void QVNCCursorPrivate::createSystemCursor(int id)
{
    ...
    switch (id) {
        ...
        case Qt::ArrowCursor:
            systemCursorTable[Qt::ArrowCursor] =
                new QVNCCursorPrivate(cur_arrow_bits, mcur_arrow_bits, 16, 16, 0, 0);
            break;

        case Qt::UpArrowCursor:
            systemCursorTable[Qt::UpArrowCursor] =
                new QVNCCursorPrivate(cur_up_arrow_bits, mcur_up_arrow_bits, 16, 16, 7, 0);
            break;

        case Qt::CrossCursor:
            systemCursorTable[Qt::CrossCursor] =
                new QVNCCursorPrivate(cur_cross_bits, mcur_cross_bits, 16, 16, 7, 7);
            break;

        case Qt::IBeamCursor:
            systemCursorTable[Qt::IBeamCursor] =
                new QVNCCursorPrivate(cur_ibeam_bits, mcur_ibeam_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeVerCursor:
            systemCursorTable[Qt::SizeVerCursor] =
                new QVNCCursorPrivate(cur_ver_bits, mcur_ver_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeHorCursor:
            systemCursorTable[Qt::SizeHorCursor] =
                new QVNCCursorPrivate(cur_hor_bits, mcur_hor_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeBDiagCursor:
            systemCursorTable[Qt::SizeBDiagCursor] =
                new QVNCCursorPrivate(cur_bdiag_bits, mcur_bdiag_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeFDiagCursor:
            systemCursorTable[Qt::SizeFDiagCursor] =
                new QVNCCursorPrivate(cur_fdiag_bits, mcur_fdiag_bits, 16, 16, 7, 7);
            break;

        case Qt::BlankCursor:
            systemCursorTable[Qt::BlankCursor] =
                new QVNCCursorPrivate(0, 0, 0, 0, 0, 0);
            break;

        // 20x20 cursors
        case Qt::ForbiddenCursor:
            systemCursorTable[Qt::ForbiddenCursor] =
                new QVNCCursorPrivate(forbidden_bits, forbiddenm_bits, 20, 20, 10, 10);
            break;

        // 32x32 cursors
        case Qt::WaitCursor:
            systemCursorTable[Qt::WaitCursor] =
                new QVNCCursorPrivate(wait_data_bits, wait_mask_bits, 32, 32, 15, 15);
            break;

        case Qt::SplitVCursor:
            systemCursorTable[Qt::SplitVCursor] =
                new QVNCCursorPrivate(vsplit_bits, vsplitm_bits, 32, 32, 15, 15);
            break;

        case Qt::SplitHCursor:
            systemCursorTable[Qt::SplitHCursor] =
                new QVNCCursorPrivate(hsplit_bits, hsplitm_bits, 32, 32, 15, 15);
            break;

        case Qt::SizeAllCursor:
            systemCursorTable[Qt::SizeAllCursor] =
                new QVNCCursorPrivate(size_all_data_bits, size_all_mask_bits, 32, 32, 15, 15);
            break;

        case Qt::PointingHandCursor:
            systemCursorTable[Qt::PointingHandCursor] =
                new QVNCCursorPrivate(phand_bits, phandm_bits, 32, 32, 0, 0);
            break;

        case Qt::WhatsThisCursor:
            systemCursorTable[Qt::WhatsThisCursor] =
                new QVNCCursorPrivate(whatsthis_bits, whatsthism_bits, 32, 32, 0, 0);
            break;
        case Qt::BusyCursor:
            systemCursorTable[Qt::BusyCursor] =
                new QVNCCursorPrivate(busy_bits, busym_bits, 32, 32, 0, 0);
            break;

        case Qt::OpenHandCursor:
            systemCursorTable[Qt::OpenHandCursor] =
                new QVNCCursorPrivate(openhand_bits, openhandm_bits, 16, 16, 8, 8);
            break;
        case Qt::ClosedHandCursor:
            systemCursorTable[Qt::ClosedHandCursor] =
                new QVNCCursorPrivate(closedhand_bits, closedhandm_bits, 16, 16, 8, 8);
            break;
        default:
...
void QVNCCursorPrivate::set(Qt::CursorShape id)
{
    QVNCCursorPrivate *cursor = 0;
...
void QVNCCursorPrivate::set(const uchar *data, const uchar *mask,
